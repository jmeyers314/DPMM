...
def test_multivariate_t_density():
    ...
    r = dblquad(lambda x, y: prior.multivariate_t_density(nu, mu, Sig, [x, y]),
                -np.inf, np.inf, lambda x: -np.inf, lambda x: np.inf)
    np.testing.assert_almost_equal(r[0], 1.0, 5,
    ...
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')
        xbar = dblquad(lambda x, y: prior.multivariate_t_density(nu, mu, Sig, [x, y])*x,
                       -np.inf, np.inf, lambda x: -np.inf, lambda x: np.inf)
        ybar = dblquad(lambda x, y: prior.multivariate_t_density(nu, mu, Sig, [x, y])*y,
                       -np.inf, np.inf, lambda x: -np.inf, lambda x: np.inf)
    ...
    xy1 = np.r_[0.0, 0.1]
    xy2 = np.r_[0.2, 0.3]
    pr1 = [prior.multivariate_t_density(nu, mu, Sig, xy1),
           prior.multivariate_t_density(nu, mu, Sig, xy2)]
    xys = np.vstack([xy1, xy2])
    pr2 = prior.multivariate_t_density(nu, mu, Sig, xys)
